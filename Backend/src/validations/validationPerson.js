const validarPersona = (data) => {
  ...
  const errors = [];
  const expresionString = /^[A-Za-z\s]+$/i; //ADMITE MAYUS,MINUS,ESPACIO A-Z-a-z
  const expresionNumber = /^([0-9])*$/; //ADMITE SOLO NUMEROS 0-9 Y MUCHOS DIGITOS
  //PARA VALIDAR LOS CAMPOS NOMBRE
  if (typeof Nombre !== "string")
    errors.push({
      text: "Debe contener caracteres el nombre",
    });

  if (!expresionString.test(Nombre))
    errors.push({
  ...
  if (!expresionString.test(Apellido))
    errors.push({
  ...
  if (!expresionNumber.test(Cedula))
    errors.push({
  ...
  if (!expresionNumber.test(Telefono))
    errors.push({
